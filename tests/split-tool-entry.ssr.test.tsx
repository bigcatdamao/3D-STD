...
describe('M1.7.9 主动拆件入口', () => {
  it('画布工具栏固定显示明确的拆件按钮', () => {
    const html = renderToString(<Toolbar onOpenSplit={() => {}} onOpenAutoSplit={() => {}} />);
    expect(html).toContain('data-testid="split-tool-entry"');
    expect(html).toContain('✂ 手动拆件');
    expect(html).toContain('data-testid="auto-split-tool-entry"');
    expect(html).toContain('✦ 自动拆件');
