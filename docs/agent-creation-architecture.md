...
M1.16b 将需求理解改为“自然对话优先”：自由输入是主路径，快捷选项只是建议；每轮最多追问一个会改变结果的问题。用户可把姿态等非关键项交给 Agent 形成可编辑假设，不再被固定问卷阻塞。
...
| 阶段 | 默认模型/服务 | 输入 | 输出 | 权限 |
| --- | --- | --- | --- | --- |
| 需求澄清 | AIHubMix `deepseek-v4-flash`（非思考模式） | 文本、对话、Brief、参考图数量 | 回复、最多 1 个问题、Brief | 只读规划 |
...
| 生成文字视觉方案 | 不可以自动；用户点击 | “确认 Brief 并生成视觉方案” | 无 |
...
- M1.16b 已用 DeepSeek V4 Flash 的 Chat Completions + JSON Object 接通自然对话，并在 Worker 端继续做严格契约校验；
- 视觉方向仍使用独立的 `gpt-5.6-sol` strict-schema 路径，避免对话模型协议变化阻断后续链路；
- 已把创作链路接到现有 Hi3D 输入端；接受模型后由现有产品工作区继续完成编辑、检查与 STL 导出；
