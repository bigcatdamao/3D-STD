...
| 文档版本 | v0.6 |
| 日期 | 2026-07-23 |
| 状态 | M1.7.6 已提供用户显式触发的可调轴向候选、真实截线、闭合环指标与 GPU 只读预览；完整阶段二工具循环和真实切割仍未开放 |
...
| `preview_plane_cut` | M1.7.6 已有独立 PreviewStore、失效规则、GPU 双侧裁剪、实时 A/B 指标、真实截线、闭合环/面积 fail-closed 和每轴临时位置 | 低 | 封口、闭合临时派生网格、稳定 previewId/hash、异步 Worker 与工具协议 |
...
`inspect_model` 最接近“封装即可用”。M1.7.6 已把 `find_cut_candidates` / `preview_plane_cut` 的可调轴向与真实截面证据基线做成用户显式触发的本地能力，但还不是 Agent 后端工具循环，也不能代替真实切割几何。
...
5. **候选切面搜索（可调轴向与真实截面证据已完成）**：M1.7.6 可比较世界 X/Y/Z 轴并在 10%–90% 调整位置，对 AABB 切后尺寸、床适配、双侧入床区间和真实截线/闭合面积给出确定性证据；仍需补自动偏移搜索、结构强度、细节损伤、可隐藏接缝和装配可达性评分。
...
11. **预览生命周期（真实截面证据基线已完成）**：M1.7 已实现修复预览；M1.7.6 的切面 PreviewStore 已支持可丢弃、每轴临时位置、90ms 延迟扫描、编辑版本/床配置失效和零历史。完整阶段二仍需 previewId/hash、工具审计、Worker 取消协议与可恢复状态。
