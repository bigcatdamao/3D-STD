...
| 文档版本 | v0.6 |
| 日期 | 2026-07-23 |
| 状态 | M1.7.7 已提供用户显式触发的 27 点 Worker 扫描、几何低风险接缝推荐、真实截线与 GPU 只读预览；完整阶段二工具循环和真实切割仍未开放 |
...
| `find_cut_candidates` | M1.7.7 已有世界 AABB、打印床、X/Y/Z 10%–90% 的 27 点 Worker 扫描、双侧入床与几何低风险排序 | 低 | 非轴向候选、可靠曲率/平坦度、结构强度、可隐藏接缝与装配可达性 |
| `preview_plane_cut` | M1.7.7 已有独立 PreviewStore、失效规则、GPU 双侧裁剪、实时 A/B 指标、真实截线、闭合环/面积 fail-closed、批量 Worker 与推荐跳转 | 低 | 封口、闭合临时派生网格、稳定 previewId/hash 与后端工具桥协议 |
...
`inspect_model` 最接近“封装即可用”。M1.7.7 已把 `find_cut_candidates` / `preview_plane_cut` 的轴向批量搜索、几何评分与真实截面证据做成用户显式触发的本地能力，但还不是 Agent 后端工具循环，也不能代替真实切割几何。
...
5. **候选切面搜索（轴向批量搜索与几何评分已完成）**：M1.7.7 扫描世界 X/Y/Z 各 10%–90% 的 27 个位置，对 AABB 切后尺寸、床适配、真实截线、闭合面积、局部薄颈、轮廓复杂度、截线密度和零件比例给出确定性推荐；仍缺非轴向候选、结构强度、可靠曲率/平坦度、可隐藏接缝和装配可达性评分。
6. **鲁棒平面切割（真实求交证据，仍不产出几何）**：M1.7.6 已做世界三角形与平面求交、线段去重、闭合图、开链/分叉/共面降级和净面积；仍缺三角形分类后的新拓扑、孔环方向、封口三角化、法线/绕序修复、属性继承与派生网格验证。
...
11. **预览生命周期（Worker 推荐基线已完成）**：M1.7 已实现修复预览；M1.7.7 的切面 PreviewStore 已支持可丢弃、每轴临时位置、90ms 单点延迟扫描、27 点 Worker 批量扫描、取消/超时终止、编辑版本/床配置失效和零历史。完整阶段二仍需 previewId/hash、工具审计与可恢复后端状态。
