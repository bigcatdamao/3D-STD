...
按实现依赖排序；M1.7–M1.7.2 已补齐的基础能力同时标注，避免重复建设：

1. **多视角确定性渲染（基础已完成）**：阶段一已有前、右、顶、轴测离屏视图与视图 ID；图片哈希和阶段二可复用快照缓存仍缺。
2. **连通组件与表面区域分析（部分完成）**：M1.7.1 已有带预算的连通壳、内部壳、孤立碎片和自交诊断；M1.7.2 增加本地自交三角形对与逐条定位，但原始局部坐标不进入模型请求。组件邻接、薄颈、平坦区、曲率突变和自然接缝仍缺。
...
11. **预览生命周期（网格修复基础已完成）**：M1.7 已实现独立临时预览、可丢弃和编辑版本失效；阶段二切割预览需复用同一契约。
