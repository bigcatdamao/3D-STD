...
| 文档版本 | v0.9 |
| 日期 | 2026-07-25 |
| 状态 | M1.9.0 已开放用户直接控制、预览后确认的平面/曲面真实切割；M1.6 API/Schema 保留但不进入主导航，Agent 仍无写工具权限，合并、定位销、公差与装配验证仍未开放 |
...
8. **Agent 不得遮挡实际工具。** 能由本地确定性能力完成的动作必须有稳定、可发现的手动入口；自然语言分析只有在能缩短操作或解释证据时才允许回到产品 UI。
...
| `find_cut_candidates` | M1.7.8 已有世界 AABB、X/Y/Z 27 点扫描、双侧入床排序，以及轴向引导带内沿现有网格边的表面闭合接缝搜索 | 中低 | 任意法向/用户笔刷区域、可靠语义/强度/装配评分、局部重网格 |
| `preview_plane_cut` | M1.7.8 已有独立 PreviewStore、真实截线、表面接缝、已封口 A/B 内存临时网格、开放边验证、取消/超时和失效规则 | 中低 | 稳定 previewId/hash、鲁棒凹环三角化、自交/属性验证与后端工具桥协议 |
...
`inspect_model` 最接近“封装即可用”。M1.7.8 已把 `find_cut_candidates` / `preview_plane_cut` 的轴向批量搜索、表面接缝图割、A/B 封口临时网格与验证做成用户显式触发的本地能力，但还不是 Agent 后端工具循环，也不是可写入/可导出的生产切割。
...
按实现依赖排序；M1.7–M1.7.6 已补齐的基础能力同时标注，避免重复建设：
...
5. **候选接缝搜索（轴向批量 + 表面边图第一版已完成）**：M1.7.8 先用世界 X/Y/Z 27 点扫描确定引导中心，再在可调带宽内以面邻接最小割搜索沿已有网格边的单闭合接缝；仍缺任意法向/笔刷区域、结构强度、语义、可隐藏接缝和装配可达性评分。
6. **鲁棒切割几何（拓扑闭合预览已完成，生产验证仍缺）**：M1.7.8 已按表面分区复制源三角形并生成双侧反向中心扇形封口，验证 A/B 开放边为 0；仍缺接缝新顶点/局部重网格、凹环与非平面环约束三角化、自交、法线/属性继承、体积和可制造验证。
...
11. **预览生命周期（表面切割基线已完成）**：M1.7.8 的 PreviewStore 绑定实例、编辑版本、床、轴、引导位置与吸附带宽；表面 Worker 支持资产缓存、取消、60 秒超时、迟到结果隔离和零历史。完整阶段二仍需 previewId/hash、工具审计与可恢复后端状态。
...
### 10.0.6 M1.7.6 真实截面证据基线

- 浏览器本地把实例三角形变换到世界坐标，与当前轴向平面求真实交线；截线和顶点不进入模型请求。
- 只有完整扫描且截线图由无分叉闭合环组成时才给环数、周长和净面积；部分扫描、开链、空截面、分叉和共面三角形全部 fail-closed。
- 扫描预算为 120,000 面、唯一截线预算为 24,000 条；预算触顶只保留证据，不给面积或可制造结论。
- 滑杆的 AABB/床适配即时更新，真实截面延后 90ms；这只是前端交互调度，不是 Agent 自主工具循环。
- 当前仍没有封口网格、派生零件、previewId/hash 或写工具，不能把真实截线升级描述为 `apply_plane_cut` 已可用。

### 10.0.7 M1.7.7 几何低风险接缝基线

- 用户显式启动本地 `seam-scan.worker`，扫描世界 X/Y/Z 各 10%–90% 共 27 个位置；模型不能自主启动或循环调用。
- Worker 缓存资产 position/index 副本，只返回不含截线坐标的截面摘要；取消、异常和 45 秒超时通过终止 Worker fail-closed。
- 只有双侧入床且真实截面完整、闭合、无歧义、有正面积的样本进入排名；总分使用薄颈、面积、轮廓复杂度、截线密度和比例等可解释几何代理。
- UI 最多显示 3 个候选、评分理由、风险和滑杆标记，并明确这些指标不识别人脸、关节、受力、装配或设计语义。
- 推荐跳转只驱动现有只读预览，不创建 `previewId`、派生网格或历史项；Agent 权限表和阶段三确认门不变。

### 10.0.8 M1.7.8 表面自适应真实切割预览基线

- 轴向候选只提供搜索中心与法向；用户显式设置表面吸附带宽，本地 `surface-cut.worker` 在源网格面邻接图上运行最小割，接缝可离开轴向平面并沿现有网格边闭合。
- 源模型必须是单一连通水密边流形且不含退化面；面数上限 80,000、接缝边上限 12,000。缺少两侧种子、分叉、多环、预算或封口失败全部 fail-closed。
- 成功时按分区复制真实源三角形，并为 A/B 生成反向中心扇形封口；只有两侧拓扑开放边都为 0 才进入 ready 状态。
- 临时 BufferGeometry 只存在于 PreviewStore/视口，不创建场景节点、资产 lineage、历史或导出文件；模型不能启动 Worker，也没有 `apply_plane_cut` 权限。
- 接缝受现有网格分辨率限制；中心扇形没有凹环/非平面自交、强度、装配、属性或制造验证，因此只代表阶段二几何能力探针，不代表阶段三可执行结果。

### 10.0.9 M1.7.9 主动拆件入口与 Agent UI 降级

- 当前产品 UI 移除 `SplitAnalysisPanel` 和“AI 拆件”页签；旧页签偏好迁移到打印检查。后端 `/api/agent/split-analysis`、严格 Schema、Provider 适配和评测样本继续保留，便于未来基于证据决定是否恢复。
- 画布工具栏直接暴露用户控制的“✂ 拆件”；只接受一个选中实例，自动取得或刷新检查尺寸证据，再进入现有 PreviewStore。
- X/Y/Z、引导位置、推荐扫描与表面吸附都是显式本地操作；Agent 不调用 Worker、不替用户选方案，也不获得写权限。
- 若未来恢复阶段一 UI，必须绑定可执行动作、引用具体证据并通过 Gold Set；纯文字方案不能重新占用主导航。

### 10.0.10 M1.8.0 用户确认后的真实平面切割

- 新增的真实平面切割是用户主动进入、用户直接调参、用户点击确认的本地确定性功能，不是 Agent 工具调用；权限表中的 `apply_plane_cut` 仍保持禁用。
- 用户可通过画布 TransformControls 或右栏数值控制世界切面的 XYZ 位置、XYZ 旋转和显示框宽高；显示大小不改变无限数学平面的裁切结果。
- 独立 Worker 在资产局部空间执行任意平面三角形裁切、截面闭环重建和 A/B 反向封口；共面歧义、开放/分叉截面、无交点、封口失败和预算触顶全部拒绝写入。
- 成功后 copy-on-write 创建两个派生资产和两个实例，以单条 `split` command 原子替换源实例；撤销恢复原实例并移除 A/B，原始资产副本不修改。
- 该实现为未来 `apply_plane_cut` 提供确定性几何与历史基线，但尚缺审批票据、工具审计和 Agent 预览绑定，因此不能因底层函数存在就提升 Agent 权限。

### 10.0.11 M1.9.0 用户确认后的曲面自适应切割

- 该能力仍是用户主动操作的本地确定性工具：Agent 不选择对象、不设置导向、不触发预览，也不能点击确认。
- 任意方向导向面、表面吸附范围和接缝偏好进入独立 Worker；Worker 仅在单个连通水密网格上搜索一个沿现有网格边的无分叉闭环。
- 预览先返回真实 A/B 临时网格、公共接缝和封口指标；只有闭环、自交、封口扭曲度与 A/B 开放边验证全部通过时，UI 才显示“确认曲面切割”。
- 确认后复用 copy-on-write 与单条 `split` 历史；原始资产不改，撤销恢复源实例。曲面能力的存在不改变权限表，未来若封装 Agent 工具仍需预览票据、用户确认、审计与过期校验。
- 当前不支持自由手绘曲线、局部重网格、多闭环/分叉接缝、非平面高扭曲封口、榫卯、公差或装配验证，不得把本地曲面功能描述成高度自主拆件。
