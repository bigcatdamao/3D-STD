...
| 文档版本 | v0.4 |
| 日期 | 2026-07-22 |
| 状态 | M1.7.4 已提供用户显式触发的本地轴向候选与 GPU 只读预览；完整阶段二工具循环和真实切割仍未开放 |
...
| `find_cut_candidates` | M1.7.4 已有世界 AABB、打印床和 X/Y/Z 中线确定性候选 | 中低 | 偏移搜索、真实截面轮廓、自然接缝、薄颈、强度与细节损伤评分 |
| `preview_plane_cut` | M1.7.4 已有独立 PreviewStore、失效规则、GPU 双侧裁剪与 A/B 指标卡 | 中低 | 真实求交、封口、闭合临时派生网格、剖面轮廓、稳定 previewId/hash |
...
`inspect_model` 最接近“封装即可用”。M1.7.4 已把 `find_cut_candidates` / `preview_plane_cut` 的轴向基线做成用户显式触发的本地能力，但还不是 Agent 后端工具循环，也不能代替真实切割几何。
...
按实现依赖排序；M1.7–M1.7.4 已补齐的基础能力同时标注，避免重复建设：
...
5. **候选切面搜索（轴向基线已完成）**：M1.7.4 可比较世界 X/Y/Z 中线，对 AABB 切后尺寸、床适配、剩余超限轴和切面面积代理值评分；仍需补偏移搜索、真实截面积、结构强度、细节损伤、可隐藏接缝和装配可达性。
6. **鲁棒平面切割（仅视觉预览）**：M1.7.4 的 clipping plane 只负责分色显示，不产出几何；仍缺三角形分类、边求交、轮廓闭合、孔环识别、封口三角化、法线/绕序修复和属性继承。
...
11. **预览生命周期（轴向切面基线已完成）**：M1.7 已实现修复预览；M1.7.4 的切面 PreviewStore 已支持可丢弃、编辑版本/床配置失效和零历史。完整阶段二仍需 previewId/hash、工具审计与可恢复状态。
