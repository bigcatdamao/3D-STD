...
| `find_cut_candidates` | 包围盒、连通性分析的部分基础数据 | 低 | 连通组件、候选平面、截面轮廓、自然接缝与候选评分 |
| `preview_plane_cut` | 历史“预览冻结”概念、Three.js 渲染注册表 | 低 | 真正的平面切割、封口、临时派生网格、剖面高亮、预览 ID |
...
`inspect_model` 是唯一接近“封装即可用”的工具。其他四个都不能只靠提示词实现，必须先补确定性几何算法。
...
1. **多视角确定性渲染**：相同对象、相同背景、固定相机、无 Gizmo/床/选中描边干扰；生成视图 ID 与图片哈希。
2. **连通组件与表面区域分析**：分离壳体、组件邻接、薄颈、孔洞、平坦区、曲率突变和自然接缝。
3. **真正的局部过悬分析**：按打印方向计算面法线、过悬角、过悬面积与连通区域；不能复用现有整件离床警告。
4. **薄壁分析**：基于双向射线、SDF 或体素距离场估算局部厚度，并结合打印方式、喷嘴/光固化参数给阈值。
5. **候选切面搜索**：在床适配、截面积、结构强度、细节损伤、可隐藏接缝和装配可达性之间评分。
6. **鲁棒平面切割**：三角形分类、边求交、轮廓闭合、孔环识别、封口三角化、法线/绕序修复和属性继承。
...
10. **派生资产和副本策略**：`sourceAssetId`、`operationId`、`parentPartId`、几何哈希、预览/当前/归档状态。
11. **预览生命周期**：预览必须写入独立临时注册表，可随时丢弃；场景编辑版本变化后预览自动失效。
