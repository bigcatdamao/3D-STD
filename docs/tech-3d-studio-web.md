...
| 项 | 内容 |
|---|---|
| 版本 | v3.6(M1.11c 面组驱动真实 A/B 切割) |
| 日期 | 2026-07-29 |
| 上游文档 | 《PRD v1.00》 |
| 外部数据核实日 | 2026-07-20(Tripo 官方 Upload / Generation 文档) |

**v3.6 变更记录(M1.11c 面组驱动真实 A/B 切割)**:① **面组即切割语义**——`faceLabels` 将紫色面组固定映射为拆下件 A、未涂区域映射为保留件 B；面组模式不再调用最小割重新猜测用户意图。② **BVH 同源 Worker 输入**——真实切割直接复制画笔会话中 BVH 重排后的几何和面组掩码；标签每轮单独复制后作为 Transferable 发送，既保持三角面序一致，也不会 detach 当前会话。③ **双侧共享封口**——已验证绿色唯一闭环直接成为 A/B 分界；封口投影、二维自交、扭曲阈值和三角化继续复用真实曲面内核，A/B 使用相反绕序的同一组三角形；任一侧拓扑开放边不为 0 就 fail-closed。④ **预览再确认**——`previewing` 与 `previewReady` 只保存临时 A/B，视口以紫色拆下件、绿色保留件和青绿色接缝表达结果，源实例仍保留；返回接缝不写历史。⑤ **原子提交与撤销**——确认后通过 `splitInstanceWithDerivedParts` 创建 `A 拆下件`、`B 保留件` 两个派生资产，元数据 kind 为 `face_set_surface_cut`；只写一条 split 历史，撤销恢复源实例，原始资产不被覆盖。⑥ **权限边界**——仍只支持单一连通水密源网格、一个闭环、两个零件、80,000 面真实切割预算和近似平面封口；不包含重网格、UV/材质继承、榫卯、公差、装配验证或 Agent 写权限。功能提交 `25fde26`、版本文档提交 `8a87dfa` 已推送至 `main`；Cloudflare 功能部署 `0fadf118-23ea-4935-b091-98aecc8cd178` 与外部 Chrome 首屏核验通过。

**v3.5 变更记录(M1.11b 面组边界转只读接缝预览)**:① **同源面序约束**——BVH 会重排三角形，面掩码、焊接拓扑和接缝排序继续绑定同一份拾取几何副本；会话显式注册该几何与拓扑，禁止用原 STL 面序重新解释面组。② **确定性闭环验证**——焊接边新增稳定端点 ID 与代表位置；从“已涂/未涂”交界构建边图，按顶点度数检测断点与分叉，按连通分量检测多环，再排序得到唯一闭环。面组触及开放边或非流形边时 fail-closed；退化环和二维投影自交同样拒绝进入通过态。③ **只读接缝态**——验证通过后冻结画笔，视口以 4px 绿色闭环替代黄色细边界；右键旋转、中键平移和滚轮缩放继续可用。“返回修改面组”只清空临时接缝结果，不写场景历史；`Ctrl+Z` 在只读态先返回编辑。④ **几何风险指标**——按实例世界变换计算接缝长度、Newell 平面最大偏差、已涂区域包围尺寸和 A/B 面数占比；非平面封口、过薄区域和极小侧以 warning 呈现，不伪装为已解决。⑤ **权限边界**——“M1.11c：确认切割”继续禁用；本版不创建 A/B、不封口、不持久化、不调用 M1.10b 真实曲面切割。100,000 面以上继续只显示紫色面组，不构建接缝。本记录为本地开发，不代表已提交、上传或部署。

**v3.4 变更记录(M1.11a Blender 式绘制面组本地试做)**:① **交互模型重做**——曲面入口从 M1.10b 的离散控制点改为连续面组画笔；左键拖动添加紫色面组，`Ctrl+左键` 临时擦除，`B/X` 切换模式，`[/]` 调半径；一次 pointer capture 生命周期合并为一笔本地撤销。② **高频数据不进 React**——面组掩码使用 `Uint8Array`、当前笔使用 `Map` 去重；鼠标移动由原生 canvas 事件和 `requestAnimationFrame` 合帧，采样时直接更新 GPU color attribute，Zustand 只在松手后发布统计和 revision。右/中键按下时停止画笔 hover raycast，OrbitControls 继续独占旋转/平移，滚轮仍直接缩放。③ **BVH 与连续表面约束**——新增 `three-mesh-bvh`；工具打开时为几何建立 BVH，画笔只遍历半径内三角形，再从实际命中面沿共享边筛出同一连续、朝向相近的表面，避免薄件背面和空间邻近壳误染。拾取、紫色覆盖层和边界拓扑统一使用 BVH 重排后的面序，消除索引错位。④ **即时边界与高面数降级**——松手后按焊接边邻接提取“已涂/未涂”交界并显示黄色线；超过 100,000 面跳过显式边界构建，只保留实时色块，防止一次笔划后的主线程长停顿。⑤ **权限与版本边界**——面组只存在于当前拆件会话，不写场景历史、不持久化、不创建 A/B；“生成接缝”明确禁用。M1.10b 的真实曲面切割底层仍保留，但本版不把面组接到它；面组边界转接缝归 M1.11b，确认后真实拆分归 M1.11c。本记录为本地试做，不代表已上传或部署。

**v3.3 变更记录(M1.10b 贴面曲线控制点本地试做)**:① **直接贴面编辑**——曲面切割不再复用定位平面和 W/E/R 手柄，新增 `ManualSurfaceCurveEditor`；左键射线命中目标模型添加世界空间控制点，拖动控制点时持续回射并吸附到同一表面，右键视角旋转保留。② **曲线成为真实几何引导**——`surfaceGuidePoints` 经状态、Runner、协议和 Worker 传入 `surface-cut-core`；最小割的引导代价从“距单一平面”改为优先使用“距闭合折线”，拟合平面只建立 A/B 硬种子，避免接缝跳到远处自动收腰。③ **预览确认链路复用**——至少三点后闭合并计算；通过后继续复用蓝/紫真实 A/B、黄色公共接缝、共享封口、copy-on-write 派生资产和单条撤销历史。④ **产品入口收口**——“✂ 拆件”默认进入贴面曲线，平面切割保留为第二页签；右栏仅显示落点、拖点、撤销/清空、主按钮和折叠的贴线半径/偏好。⑤ **边界**——仍只支持单连通水密流形、单闭环、80,000 面、沿现有网格边和近似平面封口；尚无连续画笔、曲线平滑、局部重网格、强弯曲封口、榫卯或公差。本记录为本地试做，不代表已上传或部署。

**v3.2.1 变更记录(M1.10a.1 曲面切割首屏精简与滚动修复)**:① **删除阻塞操作的说明**——曲面第 1 步不再显示对象长名称、定位面解释卡、XYZ 箭头教学卡和生成后三步卡；平面切割原有说明保持不变。② **主操作前置**——“生成表面闭合接缝”放在基础旋转/大小参数之后，折叠的高级寻缝参数之后移，首屏优先呈现真正操作。③ **右栏恢复滚动**——`.inspector-content` 增加 `overflow-y:auto` 与稳定滚动条槽，修复外层工作区隐藏溢出后内容无法向下访问的问题。④ **回归边界**——未修改曲面搜索、封口、确认和历史内核；1366×768 外部 Chrome 验收中生成与确认按钮均可见、启用且实际完成 A/B 切割，浏览器零警告/错误。

**v3.2 变更记录(M1.10a 两阶段曲面接缝预览)**:① **粗定位与接缝确认显式分阶段**——曲面切割编辑态定义为第 1 步，只用引导面确定接缝的大概位置；真实 Worker 结果进入第 2 步，避免用户把定位平面误解为最终切口。② **穿模粗定位预览**——新增 `ManualSurfaceGuidePreview`，用世界引导面把源几何实时呈现为蓝/紫两侧；源实例降级为上下文，删除容易被理解成切口厚度的透明搜索体积。③ **接缝态清场**——曲面 `previewReady` 时完全卸载定位平面、XYZ 手柄与通用 `TransformControls`；右栏改为独立验证界面，只保留 A/B、实际接缝、几何指标、返回和确认。④ **接缝可读性**——公共闭环由 1px `LineSegments` 改为 `LineSegments2 + LineMaterial` 的 3.5px 亮黄色屏幕线宽；面板说明、指标和操作按钮统一提高字号。⑤ **可逆的状态回退**——`returnManualSurfaceSplitToGuide()` 只清除临时结果并把状态恢复到 `editing`，不修改源实例、不写历史；确认后仍复用 copy-on-write 派生资产与单条 `split` 历史。⑥ **边界不扩张**——M1.10a 仍不提供整环表面拖动或局部控制点，最小割、网格预算、单闭环、近似平面封口和 fail-closed 约束保持 M1.9 不变。

**v3.1.1 变更记录(M1.9.1 切割手柄可靠性与接缝搜索交互)**:① **专用移动手柄**——平面/曲面移动模式改用 `ManualCutTranslateGizmo`，三轴固定为 112px 屏幕尺寸、使用 0.14 世界单位透明拾取圆柱并关闭深度遮挡；高面数模型不会盖住箭头。② **可靠结束拖拽**——位置按 0.5mm 吸附；`pointerup`、`pointercancel`、窗口失焦和鼠标按键丢失都会释放捕获与交互锁，Esc 恢复本次拖拽起点。旋转/缩放继续复用 `TransformControls`。③ **搜索带语义重构**——“表面吸附范围”改为“接缝搜索宽度”，同时显示 `±Nmm` 与总宽度 `2Nmm`，明确透明体积不删除材料、不改变切口厚度。④ **渐进式引导**——增加精确/标准/宽松三档，以及移动引导面→从标准档预览→调整后重算的三步说明；接缝偏好明确为确定性权重而非角色语义理解。⑤ **几何边界不扩张**——本版本不修改最小割、封口和 fail-closed 约束，只修复操控命中与产品表达。

**v3.1 变更记录(M1.9.0 曲面自适应切割与统一拆件操控)**:① **平面/曲面双模式**——“✂ 拆件”右栏增加平面切割与曲面切割页签；曲面模式把任意世界方向导向面转换到资产局部空间，并以吸附范围约束表面搜索，不再局限世界 X/Y/Z 轴。② **表面最小割与可解释偏好**——`surface-cut-core.ts` 在单连通水密面邻接图上执行 s-t 最小割，代价综合边长、距导向面距离、折角与局部密度；“均衡 / 最短 / 贴折痕”改变权重但不绕过闭环验证。③ **可靠封口取代中心扇形**——单个无分叉接缝投影到 Newell 法线建立的二维基底，先拒绝自交和超过 12% 的封口扭曲，再用 `THREE.ShapeUtils.triangulateShape` 生成 A/B 相反绕序封口；两侧拓扑开放边必须为 0。④ **预览后确认**——Worker 先返回真实 A/B BufferGeometry、公共接缝和指标；视口以蓝/紫双色预览，源实例只降级为上下文。用户确认后才创建两个派生资产并写一条原子 `split` 历史，撤销恢复源实例。⑤ **Blender 式操控收口**——平面与曲面的位置统一由画布 XYZ 手柄拖动，右栏不再放重复的位置滑杆；侧栏保留精确旋转、宽高联动大小、吸附带和接缝偏好。预览成功后隐藏大面积导向框以突出真实切口。⑥ **预算与边界**——曲面最多处理 80,000 源面和 4,096 个封口轮廓点，只支持单个连通水密网格与一个沿已有网格边的闭环；没有自由手绘、多环/分叉、局部重网格、属性继承、榫卯、公差或装配验证，Agent 写权限仍关闭。

**v3.0 变更记录(M1.8.0 真实平面切割与原子撤销)**:① **主动入口直接进入写前编辑模式**——单选实例点击“✂ 拆件”后，Inspector 切为专用平面切割面板，画布挂载独立 `TransformControls`；W/E/R 分别控制移动、旋转和显示大小，Esc 取消，切割期间普通对象选择、删除和对象 Gizmo 暂停。② **世界切面与实例 TRS 解耦**——切面在世界空间编辑，确认前通过实例 TRS（含非等比缩放）转换到资产局部平面；框的宽高只用于视觉定位，几何运算使用无限平面。③ **Worker 真实裁切与双侧封口**——`plane-split-core.ts` 对索引/非索引三角网格执行任意平面裁切，收集截线、重建多环/嵌套闭合截面并用约束形状三角化生成 A/B 反向封口；无交点、完全共面歧义、开放/分叉截面、封口失败、空零件和超过 2,000,000 源面一律 fail-closed。④ **copy-on-write 派生与单条历史**——`splitInstanceWithDerivedParts` 保留源资产，创建 A/B 几何、资产和实例，并在一个 `split` command 中原子替换源实例；撤销恢复源实例并删除派生资产，重做恢复 A/B。⑤ **权限与属性边界**——这是用户显式确认的确定性本地工具，不向 Agent 开放 `apply_plane_cut`；本版重算法线，只保证几何与封口，不保留 UV、切线、蒙皮和材质组，也不验证结构强度、装配公差或最终可制造性。

**v2.9 变更记录(M1.7.9 主动拆件入口与工作台)**:① **去除文字建议主界面**——`App` 不再装载 `SplitAnalysisPanel` 或订阅分析状态，Inspector 收口为属性/打印检查/历史；M1.6 API 与评测代码保留为未暴露实验能力。旧布局偏好中的 `inspectorTab: "split"` 在解析时迁移到 `check`。② **画布一级入口**——`Viewport.Toolbar` 固定提供“✂ 拆件”，依据当前选中实例数给出可操作提示；无选择和多选都 fail-visible。③ **检查数据自动编排**——入口只接受一个实例；若存在新鲜 `dims` 证据直接启动现有 PreviewStore，否则订阅 `useCheck` 并调用 `runPrintCheck`，检查完成后按实例 ID 找回尺寸证据并自动打开工作台。④ **不再依赖超床条件**——`IssueRow` 允许与当前 preview issue 匹配的任意尺寸信息承载拆件工作台；原有超床条目入口继续兼容。⑤ **三步交互不改变几何内核**——X/Y/Z、位置、推荐扫描和表面吸附继续复用 M1.7.4–M1.7.8 的确定性 Worker 与 PreviewStore；本版本只改善入口、可发现性和流程，不创建场景零件、不写历史、不导出 A/B STL。

**v2.8 变更记录(M1.7.8 表面自适应真实切割预览)**:① **轴向平面降级为搜索引导**——M1.7.7 推荐位置只确定世界轴与带宽中心；`surface-cut-core.ts` 在带宽内构建源模型的面邻接图，以 s-t 最小割搜索现有网格边上的闭合边界。边代价同时考虑世界边长、距引导中心、相邻面折角与局部三角形密度，因此可离开固定平面吸附到更短收腰或明显折痕。② **源网格先验与 fail-closed**——先焊接重复坐标并验证退化面、每条边恰有两个邻面、面图只有一个连通分量、两侧都有硬种子；结果必须是一个无分叉有向闭环，最多 80,000 源面与 12,000 接缝边。多壳、非流形、分叉、多环、预算或封口验证失败都返回明确拒绝，不抽样产出伪结果。③ **真实 A/B 临时网格**——按最小割标签复制源三角形，以接缝顶点均值生成反向绕序的双侧中心扇形封口，再以拓扑边计数验证 A/B 开放边均为 0；视口渲染实际临时 BufferGeometry，不再把 GPU clipping plane 当成最终结果。④ **独立 Worker 与生命周期**——`surface-cut.worker.ts` 缓存资产 position/index；取消、异常或 60 秒超时直接 terminate 并清缓存。PreviewStore 绑定实例、`editVersion`、轴、引导位置与吸附范围，任何条件变化都会丢弃旧结果。⑤ **能力上限显式化**——接缝只能沿现有网格边，低模精度受拓扑限制；中心扇形封口可能在高凹/非平面环上自交，尚无局部重网格、属性继承、强度、装配或可制造验证。本版不创建场景零件、不写历史、不导出、不开放 `apply_plane_cut`。

**v2.7 变更记录(M1.7.7 几何低风险接缝扫描)**:① **27 点 Worker 批量扫描**——用户显式触发后，对世界 X/Y/Z 各 10%–90% 共 27 个位置调用同一真实截面内核；`seam-scan.worker.ts` 只回传无线段坐标的摘要，主线程不承担批量三角形遍历。② **资产几何缓存与可取消收口**——首轮把 position/index 紧凑副本 Transfer 给常驻 Worker，成功后的同资产扫描只传 `assetId`、实例 TRS 和切面；取消、Worker 异常或 45 秒超时立即 `terminate` 并清空主线程缓存标记，重试安全重传。③ **可解释确定性评分**——仅让“双侧入床 + 完整无歧义闭合环 + 正面积”的样本入围；总分由入床 35、邻域薄颈 20、相对小面积 16、轮廓紧凑/少环 14、低截线密度 10、零件比例 5 组成，同轴推荐间距至少 15%。④ **推荐到预览闭环**——最多 3 个推荐卡显示理由与首要风险，当前轴同步显示滑杆标记；点击后直接切换轴/位置并只为目标位置计算完整黄色轮廓，避免先扫描无用中线。⑤ **产品边界**——截线密度只是网格细节代理，面积/紧凑度不是结构强度；UI 明示不识别人脸、机械关节、受力与装配语义。推荐、跳转和取消均不改几何、不写历史、不开放 `apply_plane_cut`。
