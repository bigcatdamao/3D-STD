...
服务层(Cloudflare Workers,单 Worker 多路由)
  ...
  ├─ /api/quota      配额查询
  ├─ Durable Object:访客配额计数、账务记录(强一致)
  ├─ KV:任务映射、熔断状态(读多写少,可容忍最终一致)
  └─ R2:AI 生成结果持久化(GLB)
外部
...
**D6 防滥用四层。** ① Cloudflare Turnstile(免费、无感)拦机器人于任务提交前;② 访客配额:浏览器指纹 + IP 复合键,3 次/日(见 §6 成本模型),计数与账务存 Durable Object——KV 为最终一致且无原子操作,并发读改写会产生配额双花,不得用于计数;③ 全局预算熔断:KV 记当日总消耗 credits,达上限(可配)后全站生成入口降级为「今日额度已用完 + 自带 key」;④ 自带 key 通道:用户 key 仅存 sessionStorage、每请求透传、服务层不落盘;⑤ 演示码:URL 携带的提升配额令牌(如 20 次/日),供面试与演示链接使用,服务层可逐码撤销——防御机制不应卡住最高价值访客。
...
| 云端 | Durable Object | 配额计数、账务记录 | 计数按日翻转 |
| 云端 | KV | 任务映射、熔断状态 | 24h TTL |
